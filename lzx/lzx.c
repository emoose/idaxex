...
int LZXinit(int window_size) {
  /* make sure window size is power of 2 */
  int window_mask = window_size - 1;
  if ((window_size & window_mask) != 0)
    return DECR_ILLEGALDATA;

  int window = 0;
  int window_test = window_size >> 1;
  while (window_test != 0)
  {
    window++;
  ...
  if (LZX(actual_size) < window_size) {
    if (LZX(window)) free(LZX(window));
    LZX(window) = NULL;
  }
  if (!LZX(window)) {
    if (!(LZX(window) = malloc(window_size))) return DECR_NOMEMORY;
